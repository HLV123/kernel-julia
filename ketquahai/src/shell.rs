...
fn cmd_help() {
    ...
    println!("  julia             -- enter Julia REPL (interactive)");
    println!("  julia <file.jl>   -- run a Julia script file");
    println!("  julia <code>      -- run Julia code directly");
    println!("");
    println!("Julia demo files:  ls /etc/julia");
}
...
// ---------------------------------------------------------------------------
// Julia Tiny v0.2 commands
// ---------------------------------------------------------------------------

/// Julia command — REPL, file runner, or one-shot code.
///
/// Usage:
///   julia                    — enter interactive REPL
///   julia /etc/julia/fib.jl  — run a .jl file
///   julia println(2+3)       — run code directly
fn cmd_julia(arg1: &str, arg2: &str) {
    if arg1.is_empty() {
        // REPL mode
        crate::forthvm::run_repl();
        return;
    }

    // Check if it's a file path
    if arg1.ends_with(".jl") || arg1.starts_with("/") {
        crate::forthvm::run_file_oneshot(arg1);
        return;
    }
    ...
    match crate::forthvm::jl_run(&mut vm, &source) {
        Ok(result) => {
            if let crate::forthvm::VmResult::Error(e) = result {
