...
fn cmd_help() {
    ...
    println!("  julia <code>      -- run Julia code (Phase 25)");
    println!("  vm <asm>          -- run VM assembly bytecode");
    println!("  vmdemo            -- run all ForthVM demos");
}
...
// ---------------------------------------------------------------------------
// Julia-Forth VM commands (Phase 25)
// ---------------------------------------------------------------------------

/// Run Julia code directly from the shell.
///
/// Usage: julia <code>
///   julia println((2+3)*4)
///   julia function fact(n) ; if n == 1 ; return 1 ; end ; return n * fact(n-1) ; end ; println(fact(6))
fn cmd_julia(arg1: &str, arg2: &str) {
    if arg1.is_empty() {
        println!("usage: julia <code>");
        println!("  julia println((2+3)*4)");
        println!("  julia i=1 ; while i<=5 ; println(i) ; i=i+1 ; end");
        println!("  julia function f(n) ; return n*2 ; end ; println(f(21))");
        return;
    }
    // Ghép arg1 + arg2 thành 1 chuỗi source
    let source = if arg2.is_empty() {
        String::from(arg1)
    } else {
        format!("{} {}", arg1, arg2)
    };

    let mut vm = crate::forthvm::ForthVm::new();
    match crate::forthvm::jl_run(&mut vm, &source) {
        Ok(result) => {
            match result {
                crate::forthvm::VmResult::Halted => {}
                crate::forthvm::VmResult::Error(e) => {
                    println!("VM error: {:?}", e);
                }
                crate::forthvm::VmResult::Yielded => {
                    println!("VM yielded (should not happen in sync mode)");
                }
            }
        }
        Err(e) => {
...
/// Run VM assembly bytecode.
///
/// Usage: vm <instructions>
///   vm PUSH 5 PUSH 6 ADD PRINT HALT
fn cmd_vm(arg1: &str, arg2: &str) {
    if arg1.is_empty() {
        println!("usage: vm <assembly>");
        println!("  vm PUSH 5 PUSH 6 ADD PRINT HALT");
        println!("  vm PUSH 10 PUSH 3 MUL PRINT HALT");
        return;
    }
    let source = if arg2.is_empty() {
        String::from(arg1)
    ...
    let mut vm = crate::forthvm::ForthVm::new();
    match crate::forthvm::assemble_into(&mut vm, &source) {
        Ok(count) => {
            serial_println!("[vm] assembled {} instructions", count);
            let result = vm.run();
            match result {
                crate::forthvm::VmResult::Halted => {}
                crate::forthvm::VmResult::Error(e) => {
...
/// Run all ForthVM demos (Phase 1-8).
fn cmd_vmdemo() {
    let mut vm = crate::forthvm::ForthVm::new();
    crate::forthvm::demos::run_all_demos(&mut vm);
}
